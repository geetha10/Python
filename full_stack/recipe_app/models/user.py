...
class User:
    ...
    def validate_registration_form(data):
        is_valid = True
        query = "SELECT * FROM users WHERE email = %(email)s;"
        results = mysqlconnection.connectToMySQL(User.db).query_db(query,data)
        print(results)
        if len(results) >= 1:
            flash("Email already exists in Data Base.","register")
            is_valid=False
    ...
    def validate_login_form(data):
        is_valid = True
        if not EMAIL_REGEX.match(data['email']):
            flash("Invalid Email. Please try again","login")
            is_valid=False
        if len(data['password']) < 1:
            flash("Please Enter the password to login","login")
            is_valid=False
        return is_valid

    @staticmethod
    def validate_update_form(data):
        is_valid = True
        if not EMAIL_REGEX.match(data['email']):
            flash("Invalid Email. Please try again","login")
            is_valid=False
